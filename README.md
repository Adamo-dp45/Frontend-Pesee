...
Le site web des clients. Il ne touche jamais la base : il appelle l'API avec le jeton de
l'utilisateur connecté, et c'est l'API qui tranche toutes les autorisations.
...
- **Bon à savoir**
    > Turbo n'affiche pas une réponse `200` à un `POST` : un formulaire doit rediriger après succès,
      ou renvoyer `422` pour être réaffiché avec ses erreurs.
    > Le filtre Twig `merge` renumérote les clés entières : les listes déroulantes indexées par
      identifiant se construisent en PHP (`GestionController::options()`), pas dans le gabarit.
